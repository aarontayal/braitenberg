...
Description: This version will introduce a better way of spawning the plants/food.  The plants will do an approximate
logistic growth. To start, we will set a max number of plants.  Each of these plants, at each time, has a location and
an alive/dead status.  If only a small fraction of the plants are alive, then every iteration, a few more plants will
become alive (with a fresh new location).  As more plants become alive, the rate at which new plants become alive
increases, to a certain point.  As the fraction of alive plants approaches 1, the rate at which dead plants become alive
goes to zero.  Eventually, there will be the maximum number of plants.  As always, the robots eat the plants and reduce
the proportion of alive plants.

The robot population can also change.  There is a certain max population of robots that can be in the habitat, but as
long as the population is lower, the robots life is dependent on whether it has energy greater than zero.
...
import evolution_sim_useful_functions_v2 as useful

# ---------------------------------------------------------------------------------------------------------------------

# World set-up parameters:
amountOfTime = 2000  # Number of iterations
...
propelAdjust = 0.05  # to adjust the propelling force that the robots' motors exert
waitTimeRep = 1  # time between possible reproduction for each indiv. robot
initRobotEnergy = 20  # the energy that each robot will have when spawned at time = 0
metabolism = -0.04
maxEnergy = 100
searchStartBoxEdge = radiusBounds[1] * 5  # distance away to start looking for a spot for offspring
phenotypeToAnalyze = 3  # this is the phenotype which will be displayed on graphs at the end
...
for f in range(initNumberOfFood):
    foodAlive[:, f] = True  # set food f to be alive for the rest of time (this might be altered later)
...
for t in range(amountOfTime-1):  # start with t = 0, go through t = (amountOfTime - 2)
    ...
    for k in alives:  # This loop will calculate the acceleration due to *********************************
        ...
        (deltaAccel, deltaAccelEnergy) = useful.motor_acceleration(k=k, alives=alives, time_t_pos_c=pos[t],
