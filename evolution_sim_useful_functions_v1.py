...
def find_random_space(dim_box, new_robot_index, time_t_pos_c, new_radius, time_t_radii, init_number_of_robots):
    ...
    new_spot = np.zeros(2, dtype=np.int16)
    while empty_space != 1:  # loop until we know that the chosen spot is good
        ...
        empty_space = 2
        for j in range(init_number_of_robots):  # look at all the other circles to test for overlap
            if k != j and AABB_collision_test(R1_x=time_t_pos_c[j, 0], R1_y=time_t_pos_c[j, 1],
                                              radius1=time_t_radii[j], R2_x=new_spot[0], R2_y=new_spot[1],
                                              radius2=new_radius):
                if dist((time_t_pos_c[j, 0], time_t_pos_c[j, 1]),
                        (new_spot[0], new_spot[1])) <= new_radius + time_t_radii[j]:  # if circle k
...
def find_space_close_by(dim_box, new_robot_index, new_radius, mother_index, time_t_pos_c, time_t_radii,
                        search_start_box_edge, init_number_of_robots):

    m = mother_index
    k = new_robot_index
    tpos_c = time_t_pos_c
    t_radii = time_t_radii
    ...
    while s_box < 2 * dim_box:
        lower_bound_x = max(tpos_c[m, 0] - (s_box / 2), 0) + new_radius
        upper_bound_x = min(tpos_c[m, 0] + (s_box / 2), dim_box) - new_radius
        lower_bound_y = max(tpos_c[m, 1] - (s_box / 2), 0) + new_radius
        upper_bound_y = min(tpos_c[m, 1] + (s_box / 2), dim_box) - new_radius
        ...
        while tries < 10:  # loop and try many spots in a given box
            ...
            empty_space = 1
            for j in range(init_number_of_robots):
                    if k != j and AABB_collision_test(new_spot[0], new_spot[1], new_radius,
                                           tpos_c[j, 0], tpos_c[j, 1], t_radii[j]) == True:
                        if dist((tpos_c[j, 0], tpos_c[j, 1]),
                                (new_spot[0], new_spot[1])) <= new_radius + t_radii[j]:  # if circle k is
    ...
    if s_box > 2 * dim_box and empty_space == 0:
        ...
        print("found spot at ", new_spot, "Took ", 10 * (s_box - search_start_box_edge) + tries,
              "tries and final sbox = ", s_box)
    return (new_spot)
...
def genotype_to_phenotype(genotype, phenotype_bounds):
    ...
    phenotype[4] = ((averaged_genotype[4] * (pb[4, 1] - pb[4, 0])) + pb[4, 0]) * phenotype[0]  # eyeDist = (number between 0 and 1) * radius
...
def initialize_robot(new_robot_index,  genotype_1, genotype_2, phenotype_bounds, time_t_pos_c, time_t_radii, dim_box,
                     v_mag_init, init_number_of_robots, init_robot_energy, mother_index='none',
                     search_start_box_edge='none'):
    ...
    if search_start_box_edge == 'none':
        position[0:2] = find_random_space(dim_box=dim_box, new_radius=resultant_phenotype[0],
                                          init_number_of_robots=init_number_of_robots, new_robot_index=d,
                                          time_t_radii=time_t_radii, time_t_pos_c=time_t_pos_c)
    else:
        position[0:2] = find_space_close_by(dim_box=dim_box, new_radius=resultant_phenotype[0],
                                            search_start_box_edge=search_start_box_edge,
                                            init_number_of_robots=init_number_of_robots, mother_index=mother_index,
                                            new_robot_index=d, time_t_pos_c=time_t_pos_c, time_t_radii=time_t_radii)
    ...
    velocity[0:2] = polar_to_rect(v_mag_init, init_vel_angle)  # Set initial x and y component
    # of the velocity vector for robot k at time = 0
...
def evaluate_collisions(alive_index, time_t_phen_c, time_t_pos_c, time_t_vel_c, time_t_accel_c,
    ...
    wall = bouncing_off_wall(tpos_c[k, 0], tpos_c[k, 1], tphen_c[k, 0], dim_box, tvel_c[k, 0], tvel_c[k, 1], wall_bounce_multiplier)
    ...
    taccel_c[k, 0:2] = (taccel_c[k, 0:2] + (wall[0:2]))
    ...
    if tcollision_c[k, 1] == -1:  # if the circle a is not (so far as we know so far) colliding with another circle
        for j in range(init_number_of_robots):  # j is the "other" circle to test for collision with the current circle
            if (k != j):  # if the current circle is not the same as the other circle
                if AABB_collision_test(tpos_c[k, 0], tpos_c[k, 1], tphen_c[k, 0], tpos_c[j, 0], tpos_c[j, 1], tphen_c[j, 0]) == True:
                    # AABB collision test to see if the bounding boxes of a and j are overlapping.
                    # https://gamedevelopment.tutsplus.com/tutorials/when-worlds-collide-simulating-circle-circle-collisions--gamedev-769

                    center_distance = dist((tpos_c[k, 0], tpos_c[k, 1]), (tpos_c[j, 0], tpos_c[j, 1]))  # distance between centers of
                    # circles k and j
...
def motor_acceleration(alive_index, time_t_pos_c, time_t_vel_c,
                       time_t_phen_c, min_dist_to_eye_squared, max_dist_to_activate, friction_const,
                       init_number_of_robots, energy_of_k, food_pos, time_t_eaten, propel_adjust=1):
    k = alive_index
    tpos_c = time_t_pos_c
    tvel_c = time_t_vel_c
    ...
    if energy_of_k > 0:
        ...
        for j in range(init_number_of_robots):
            if j != k:
                if AABB_collision_test(tpos_c[k, 0], tpos_c[k, 1], max_dist_to_activate,
                                       tpos_c[j, 0], tpos_c[j, 1], 0) == True:  # if robot k and robot j are close enough
                    ...
                    l_eye_activ += 2 / (l_eye_dist_squared)  # ----------------- a robot activates the eye twice as
                    # much as a food particle
                    r_eye_activ += 2 / (r_eye_dist_squared)
        ...
        for f in range(food_pos.shape[0]):
            if time_t_eaten[f] == False:
                l_eye_dist_squared = dist_squared(tpos_c[k, 3:5], food_pos[f])
                r_eye_dist_squared = dist_squared(tpos_c[k, 5:7], food_pos[f])
                if l_eye_dist_squared < min_dist_to_eye_squared:
                    l_eye_dist = min_dist_to_eye_squared
                if r_eye_dist_squared < min_dist_to_eye_squared:
                    r_eye_dist = min_dist_to_eye_squared
                l_eye_activ += 1 / (l_eye_dist_squared)
                r_eye_activ += 1 / (r_eye_dist_squared)

        l_propel = r_eye_activ * propel_adjust
        r_propel = l_eye_activ * propel_adjust

        """
        l_propel = (np.random.rand()-0.5)/10
        r_propel = (np.random.rand()-0.5)/10
        """
        """
        l_propel = 0.1
        r_propel = 0.11
    ...
    delta_energy = -(l_propel + r_propel)  # Note: should improve the physics of using energy.  Should delta_energy
    # also depend on propel_adjust, since that is how actual acceleration is calculated?
...
def eat_food(food_pos, time_t_eaten, time_t_plus_1_eaten, food_energy, food_radius, robot_center_xy, robot_radius):
    delta_food_energy = 0
    total_number_food = food_pos.shape[0]
    for f in range(total_number_food):
        if (time_t_eaten[f]) == False:
            if (time_t_plus_1_eaten[f]) == False:
                if AABB_collision_test(R1_x=robot_center_xy[0], R1_y=robot_center_xy[1], R2_x=food_pos[f, 0],
                                       R2_y=food_pos[f, 1], radius1=robot_radius, radius2=food_radius):
                    if dist(robot_center_xy, food_pos[f]) < food_radius + robot_radius:
                        time_t_plus_1_eaten[f] = True
                        delta_food_energy += food_energy
        else:
            time_t_plus_1_eaten[f] = True  # if robot was eaten (ie. is not in the habitat) at time t, then it most
            # definitely is not present at time t + 1
    return(time_t_plus_1_eaten, delta_food_energy)