...
def initialize_robot(new_robot_index, genotype_1, genotype_2, phenotype_bounds, robot_positions,
    ...
    if pos_to_be_born_near == 'none':
        # TODO: python and numpy disagree on whether it is okay to compare a numpy array
        # to a string.  Right now, if pos_to_be_born_near is a numpy array, then pos_to_be_born_near == "none" returns
        # False.  But this might change in future versions of Python.  Thus, Python gives us a "FutureWarning"
        position[0:2] = find_random_space(dim_box=dim_box, new_radius=resultant_phenotype[0],
                                          new_robot_index=d, robot_radii=robot_radii,
                                          robot_positions=robot_positions, alives=alives)
    else:
...
def motor_acceleration(k, alives, time_t_pos_c, time_t_vel_c, robot_energy, init_robot_energy,
                       ...
                       k_biases_0th_lay, k_biases_1st_lay, propel_adjust, rob_eye_activ_adjust, food_eye_activ_adjust):
    tpos_c = time_t_pos_c
    tvel_c = time_t_vel_c
    ...
    k_sensor_neurons = np.array((robots_l_eye_activ/rob_eye_activ_adjust,
                                 robots_r_eye_activ/rob_eye_activ_adjust,
                                 food_l_eye_activ/food_eye_activ_adjust,
                                 food_r_eye_activ/food_eye_activ_adjust,
                                 robot_energy/init_robot_energy))  # All the sensor neurons have positive values,
    # but they are NOT necessarily scaled to between 0 and 1
