...
def sigmoid(x):  # x might be a vector, and in that case we want to apply sigmoid to each of the components of the
    # vector.  Python already does this.
    return(1/(1+np.exp(-x)))  # range of output: 0 to 1

def zero_zero_sigmoid(x):
    return(-1 + 2*(1/(1+np.exp(-x))))  # range of output: -1 to 1
...
def RELU(x):
    if isinstance(x, np.ndarray):
        ...
        print("error: RELU function doesn't know what type is the input")
...
def get_motor_acceleration(robot_angle, robot_mass, left_friction_x, left_friction_y, right_friction_x,
    ...
    Very important note: Exactly (1/2) of the robot_mass is considered to be held in the left motor, and half in the
    right.  The motors lie on the diameter of the robot that splits the robot into front and back.
    ...
    It is also important to note that the propelling force for each motor is pointing in a direction perpendicular to
    the front/back diameter of the robot.  Thus, the robot_angle influences the x and y components of the propelling
    force for each of the left and right motors."""
...
def initialize_robot(new_robot_index, genotype_1, genotype_2, phenotype_bounds, robot_positions,
                     ...
                     pos_to_be_born_near, search_start_box_edge="none"):
    d = new_robot_index
    ...
    if isinstance(pos_to_be_born_near, np.ndarray):
...
def evaluate_collisions(k, alives, time_t_phen_c, time_t_pos_c, time_t_vel_c, time_t_accel_c,
    ...
    taccel_c[k, :] = (0, 0, 0)  # Theoretically, there is zero accel to start.  This command is technically unnecessary
...
def motor_acceleration(k, alives, time_t_pos_c, time_t_vel_c, robot_energy, init_robot_energy,
                       time_t_phen_c, min_dist_to_eye_squared, friction_const, max_dist_to_activate_robot, max_dist_to_activate_food,
                       init_number_of_robots, time_t_food_pos, time_t_food_alive, k_weights_0th_lay, k_weights_1st_lay,
                       k_biases_0th_lay, k_biases_1st_lay, propel_adjust):
    tpos_c = time_t_pos_c
    tvel_c = time_t_vel_c
    tphen_c = time_t_phen_c

    robots_l_eye_activ = 0
    robots_r_eye_activ = 0
    food_l_eye_activ = 0
    food_r_eye_activ = 0
    ...
    for j in alives:
        if j != k:
            if AABB_collision_test(tpos_c[k, 0], tpos_c[k, 1], max_dist_to_activate_robot,
                ...
                robots_l_eye_activ += 1 / (l_eye_dist_squared)
                robots_r_eye_activ += 1 / (r_eye_dist_squared)
    ...
    for f in np.array(np.where(time_t_food_alive == True))[0, :]:
        if AABB_collision_test(tpos_c[k, 0], tpos_c[k, 1], max_dist_to_activate_food,
                ...
                if r_eye_dist_squared < min_dist_to_eye_squared:
                    r_eye_dist_squared = min_dist_to_eye_squared
                food_l_eye_activ += 1 / (l_eye_dist_squared)
                food_r_eye_activ += 1 / (r_eye_dist_squared)

    # print("robots_l_eye_activ = ", robots_l_eye_activ, " and food_l_eye_activ = ", food_l_eye_activ)

    if robot_energy < init_robot_energy:
        robot_energy_boolean = 0
    else:
        robot_energy_boolean = 1

    k_sensor_neurons = np.array((robots_l_eye_activ * min_dist_to_eye_squared,
                                 robots_r_eye_activ * min_dist_to_eye_squared,
                                 food_l_eye_activ * min_dist_to_eye_squared,
                                 food_r_eye_activ * min_dist_to_eye_squared,
                                 robot_energy_boolean))  # All the sensor neurons have positive values,
    # but they are only approximately scaled to between 0 and 1 by scaling by the fact that the nearest distance^2 from an eye
    # to another object is no less than min_dist_to_eye_squared, and that leads to an activation of 1.  Obviously if multiple
    # objects were near the robot's eye, the activation could be greater than 1.

    if np.all(k_sensor_neurons >= 0):
        pass
    else:
        print("problem: sensor neurons negative: ", k_sensor_neurons)  # TODO: troubleshoot negative values of sensor neurons

    k_inter_neurons = RELU(np.matmul(k_weights_0th_lay, k_sensor_neurons) + k_biases_0th_lay)
    k_motor_neurons = RELU(np.matmul(k_weights_1st_lay, k_inter_neurons) + k_biases_1st_lay)

    l_propel = k_motor_neurons[0] * propel_adjust
    #print("l_propel = ", l_propel)
    r_propel = k_motor_neurons[1] * propel_adjust

    delta_energy = -(l_propel + r_propel)  # TODO: should think about the physics of using energy.
    ...
    return (motor_accel, delta_energy, k_sensor_neurons, k_inter_neurons, k_motor_neurons)
